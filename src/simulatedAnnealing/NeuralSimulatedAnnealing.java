...
public class NeuralSimulatedAnnealing {
	...
	private double startTemp=20;
	private double stoptemp=50;
	private int cycles=25;
	...
	public void iterate() throws Exception{
		...
		boolean found=false;
		int c=0;
		double error=this.network.getPeError();
		// double pcError=error;
		do{
			do{
				...
				/*do{
					pcError=error;*/
					error=this.network.iteration(false);
				/*	System.out.println("Simulated annealing iteration deviation rate: "+error);
				}while(error<pcError);*/
				...
				if(error<bestError){
					...
					found=true;
					System.out.println("Current simulated annealing best deviation rate "+bestError+" with heat "+this.currentTemp +" and cycles "+this.cycles);
				}
				c++;
		...
		do{
			do{
				...
				if(error<bestError){
					...
					this.bestSet=(ArrayList<Neuron>) ObjectCloner.deepCopy(this.network.getNeurons());
					found=true;
					System.out.println("Current simulated best error "+bestError+" with heat "+this.currentTemp +" and cycles "+this.cycles);
				}
				c++;
